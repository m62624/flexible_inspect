...
#[async_trait]
impl<'a, C, IC> ValidatorBase<C, IC, &'a str> for TemplateValidator<IC, &'a str>
where
    C: CartridgeBase<&'a str> + Debug + Sync,
...
#[async_trait]
impl<'a, C, IC> ValidatorBase<C, IC, Arc<str>> for TemplateValidator<IC, Arc<str>>
where
    C: CartridgeBase<Arc<str>> + Debug + Sync,
    IC: IntoIterator<Item = C> + AsRef<[C]> + Sync + Send,
{
    fn new(cartridges: IC) -> Self {
        Self {
