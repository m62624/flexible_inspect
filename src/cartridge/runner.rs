...
/*
   ...
   проверять все элементы через `Rule::run` и только потом проверять `NextStep::Error(value)`, в `for` в нем сразу проходят проверка `if`, так мы сразу получим `NextStep::Error(value)` для достижения раннего возврата
*/
