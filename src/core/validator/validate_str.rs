...
#[async_trait]
impl<'a, C, IC> ValidatorBase<Rule, C, IC, Arc<str>> for TemplateValidator<IC, Arc<str>>
where
    C: CartridgeBase<Rule, Arc<str>> + Debug + Sync,
    IC: IntoIterator<Item = C> + AsRef<[C]> + Sync,
{
    fn new(cartridges: IC) -> Self {
        Self {
    ...
    fn validate(&self, data: Arc<str>) -> Result<(), Vec<PystvalError>> {
        trace!("{:#?}", &self.cartridges.as_ref().iter());
        let mut error = Vec::new();
        for cartridge in self.cartridges.as_ref().iter() {
    ...
    async fn async_validate(&self, data: Arc<str>) -> Result<(), Vec<PystvalError>> {
        self.validate(data)
    }
}
