...
mod fn_find_captures {
    ...
    fn find_captures_t_7() -> PyResult<()> {
        pyo3::prepare_freethreaded_python();
        let text = "lock 1";
        let rule = rule::Rule::spawn(r"(?P<test_group>\w+ (?=1))", MatchRequirement::MustBeFound)?;
        assert_eq!(
            captures::CaptureData::find_captures(&rule, text).hashmap_for_error,
            HashMap::from([
                ("main_capture".into(), "lock ".into()),
...
mod fn_is_some {
    ...
    /// Проверка is_some если есть совпадение
    #[test]
    ...
    /// Проверка is_some если нет совпадения
    #[test]
    fn is_some_t_1() -> PyResult<()> {
        pyo3::prepare_freethreaded_python();
        let text = "lock";
        let rule = rule::Rule::spawn(r"(\d+)", MatchRequirement::MustBeFound)?;
        assert!(!captures::CaptureData::find_captures(&rule, text).is_some());
        Ok(())
    }
}
