...
impl Rule {
    ...
    /// модификаторы. А не отобранные, могут иметь те же модификтаоры `MustBeFound` и MustNotBefound. Ведь если какой-то паттерн не найден и указан `MustBeFound` то это ошибка, потому мы их проверяем во втором этапе. 
    ///
    /// Этап 3
    ...
    pub fn run(rule: &Rule, text: &str) -> NextStep {
        // Исопльзуем коллекцию с двусторонней очередью для итерационного запуска
