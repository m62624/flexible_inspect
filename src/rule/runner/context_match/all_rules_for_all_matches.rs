...
impl Rule {
    ...
    pub fn all_rules_for_all_matches(stack: &mut VecDeque<(&Rule, CaptureData)>) -> NextStep {
        ...
        // Начнем проход по стеку, который мы получили в параметрах функций, `stack_temp` будет расширять `stack` который мы получили сверху, при выполнении условий
            ...
            match Self::next_or_data_for_error(frame.0, &mut frame.1) {
                NextStep::Go => {
                        ...
                        .simple_rules
                    {
                        ...
                        // Если при прошлом цикле, мы не получили не одной ошибки, то мы можем загрузить эти правила в стек, держим в голове, что условие :
