...
/// Указываем то, что любой класс который наследуется от `PystvalException`
/// должен переопределить атрибуты `message` и `rules`

/*
   Почему именно такая реализация ?
   `PyO3` в настоящее время использует функцию `PyErr_NewExceptionWithDoc` из `CPython` для создания типов исключений в модуле `PyErr::new_type`.
   Однако эта функция создает только классы исключений `Python`, которые имеют ограниченные возможности для добавления или изменения атрибутов экземпляра.
   Аргументы функции `PyErr_NewExceptionWithDoc` предназначены только для определения переменных и методов класса `dict`, и не могут быть использованы для добавления атрибутов экземпляра.
*/

#[cfg(not(tarpaulin_include))]
pub fn export_base_error() -> String {
    format!("
